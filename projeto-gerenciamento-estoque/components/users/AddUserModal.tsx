...
const AddUserModal: React.FC<AddUserModalProps> = ({ isOpen, onClose, onAddUser }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.email || !formData.password_param || !formData.confirmPassword_param) {
      setError('Todos os campos, exceto "Forçar troca de senha", são obrigatórios.');
      return;
    }
    ...
    setFormData({ 
        ...
        role: 'Usuário comum', status: 'Ativo', forcePasswordChange: false 
    }); 
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Adicionar Novo Usuário" footer={modalFooter}>
      <form id="addUserForm" onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="role" className="block text-sm font-medium text-gray-700">Tipo (Role)</label>
