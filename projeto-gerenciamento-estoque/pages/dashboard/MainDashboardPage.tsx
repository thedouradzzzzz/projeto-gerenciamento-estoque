      ...
      <div className="bg-white p-6 rounded-xl shadow-lg">
        ...
        {hasAnyLowStockAlerts && COMPANY_OPTIONS.map(companyOpt => {
          ...
          if (!alerts || alerts.length === 0) {
            // Only show "no alerts for this company" if there are *some* alerts globally.
            // If there are no alerts at all, the message above handles it.
             if (hasAnyLowStockAlerts) {
                return (
                    <div key={companyOpt.value} className="mb-4">
                        <h3 className="text-lg font-medium text-gray-600 mb-1 border-b pb-1">{companyOpt.label}</h3>
                        <p className="text-sm text-gray-500">Nenhum produto com estoque baixo para esta empresa.</p>
                    </div>
                );
             }
             return null; 
